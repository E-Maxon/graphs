...
void Graph<int>::addEdge(int v, int u, int w, int edgeNum) {
    g[v].push_back(Edge_p<int>(v, u, w, w, edgeNum));
}


template <>
void Graph<double>::addEdge(int v, int u, int w, int edgeNum) {
    g[v].push_back(Edge_p<double>(v, u, sqrt(w), w, edgeNum));
}
...
void normalizeCycle(Cycle<T> &c) {
    std::vector<Edge_p<T>> nw(c.tr.size());
    int mni = 0;
...
template <typename T>
void dfsCycles(int v, Graph<T> &g, std::vector<int> &used, std::vector<Edge_p<T>> &currentTraversal, std::set<Cycle<T>> &allCyclesSet) {
    // std::cerr << v << "\n";
    if (currentTraversal.size() >= 2) {
        if (currentTraversal[0].v == currentTraversal.back().u) {
            ...
            std::vector<Edge_p<T>> nwCycle(currentTraversal.begin(), currentTraversal.end()); 
...
std::vector<Cycle<T>> getAllCycles(Graph<T> &g) {
    ...
    std::vector<Edge_p<T>> currentTraversal;
    for (int i = 0; i < n; ++i) {
